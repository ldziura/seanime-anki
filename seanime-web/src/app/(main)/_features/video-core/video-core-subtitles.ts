...
export type SubtitleManagerAutoSyncEvent = CustomEvent<{
    ...
    /**
     * Non-null when a seam was found AND the pre-seam cues were rewritten in the track
     * content. The rewrite is expressed RELATIVE to `offsetSeconds`, so that value is no
     * longer a free parameter — a listener must apply it even if it would otherwise defer
     * to a stored offset, or the pre-seam segment ends up shifted against the wrong
     * baseline. Carries the measurements so the correction can be surfaced after the fact;
     * a piecewise alignment is otherwise invisible once a toast has faded.
     */
    appliedSplit: SplitAlignment | null
...
`
    ...
    async autoSyncSubtitles(): Promise<SyncSelection | null> {
        ...
        let appliedSplit: SplitAlignment | null = null
        if (verdict.accept) {
